...
Vercel's Python runtime automatically detects a named `app` instance in
`api/index.py` and wraps it in its own ASGI handler — no uvicorn needed.

CORS + live-data architecture
-----------------------------
...
- Vercel's FastAPI ships CORSMiddleware (allow_origins=["*"], verified) so it
  ...
  (vercel.json excludeFiles) and the heavy ML deps, so it can only serve a
  small stale snapshot on its own.
- The fix: this entrypoint wraps the app in a tiny ASGI proxy. Browser
  requests hit Vercel (CORS-correct), Vercel forwards them server-to-server
  to Northflank (no CORS constraint server-side), and streams the live
  response back with proper CORS headers. If the upstream is unreachable we
  fall back to the local app (bundled snapshot) so nothing breaks.

The proxy lives ONLY in this file, which Northflank never loads
(render.yaml starts `uvicorn mandi_rdd.api.main:app`), so there is no
proxy loop.
"""
...
class LiveProxy:
    """ASGI middleware: forward to Northflank; fall back to the local app."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        skip = {"host", "content-length", "connection", "accept-encoding", "accept-encoding"}
        fwd = {}
        for k, v in scope.get("headers", []):
        ...
        try:
            ...
            return
        except Exception as e:  # upstream down / timeout → local fallback
            print(f"[proxy] upstream {PROXY_TARGET} failed ({e}); using local app")
            if body is not None and scope["method"] in ("POST", "PUT", "PATCH"):
                # Re-inject body for the local app: wrap receive to replay it.
                sent = [False]
...
app = LiveProxy(app)  # wrap: live upstream first, local snapshot fallback
