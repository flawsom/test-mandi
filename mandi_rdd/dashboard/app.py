...
try:
    ...
    st.error(
        ...
        "dashboard. The root file pins `plotly==6.9.0`, so this means one of:\n\n"
        "1. **A `pyproject.toml` or `uv.lock` exists at the repo root** — Cloud "
        "installs from the lockfile and **ignores requirements.txt**. (This repo had "
        "one; it was removed in 2026-08, so re-deploy from the current `master`.)\n"
        "2. **Stale dependency cache** — click the **⋮ (kebab) menu → Rebuild** in "
        "the Streamlit Cloud dashboard to force a fresh `pip install -r "
        "requirements.txt`.\n\n"
        "Also confirm the app is deployed from branch **`master`** with main file "
        "**`mandi_rdd/dashboard/app.py`** (never `gh-pages`, which has no "
        "requirements.txt)."
    )
    st.stop()
