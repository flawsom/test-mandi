...
## 6. Live Pipeline DAG — Auto-Generated Timing Diagram
...
| Step | Every Hour (~30s) | Every 24h (~5min) |
|------|:---:|:---:|
| `fetch_prices` — data.gov.in API | ✅ | ✅ |
| `fetch_rainfall` — IMD grids | ❌ | ✅ |
| `fetch_ndvi` — Sentinel Hub | ❌ | ✅ |
| `historical_backfill` — CSV archive | ❌ | ✅ |
| `backfill_state` — district mapping | ✅ | ✅ |
| `rdd_analysis` — causal estimates | ❌ | ✅ |
| `forecast_training` — seasonal naive | ❌ | ✅ |
| `classifier_training` — spike risk | ❌ | ✅ |
| `nightly_narratives` — AI summaries | ❌ | ✅ (key set) |
...
When `last_step_timings.json` exists (it gets written after every successful hourly run), the diagram shows individual wall-clock times on the relevant nodes:
...
|------|---------------------------|-------|
| `fetch_prices` | Ingestion -> A1 node | Duration tag `[12.3s]` |
| `fetch_rainfall` | Ingestion -> A2 node | Duration tag `[8.7s]` |
| `fetch_ndvi` | Ingestion -> A3 node | Duration tag `[183.5s]` (slowest — 605 districts) |
| `rdd_analysis` | Analysis -> C1 node | Duration tag `[45.1s]` |
| `forecast_training` | Analysis -> C2 node | Duration tag `[23.8s]` |
| `classifier_training` | Analysis -> C3 node | Duration tag `[15.2s]` |

If a step was skipped (e.g., rainfall only runs in the 24h full cycle), no duration tag appears for that node.

### How the diagram is generated

`scripts/generate_pipeline_diagram.py` queries the live DuckDB for actual row counts and forecast status, reads `last_hourly_run.json` for the most recent cycle's timing and outcome, and reads `last_integrity_check.json` for data quality status. It renders all of this into a Mermaid flowchart with:

- **Stage clusters** — Ingestion → Warehouse → Analysis → AI → Serving
- **Live row counts** — actual numbers from DuckDB, not hardcoded
- **Per-step model counts** — how many RDD results, forecast models, etc.
- **Timing annotation** — last run UTC timestamp and duration in the title
- **Outcome indicator** — success/failure status color-coded
- **Integrity overlay** — pass/warn/fail from the last integrity check

To inspect the latest diagram locally:
```bash
...
The diagram auto-regenerates after every successful hourly run (see `run_hourly.py` → `_run_diagram_generator()`), so it always reflects the latest state of the pipeline.

---

## 7. Visual Polish & Creative Studio Aesthetic
